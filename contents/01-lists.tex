...
\begin{itemize}
    \item \code{\begin{itemize}}: 开启无序列表环境。
    \item \code{\item}: 开始一个新列表项。
    \item \code{[label]}: 可选参数，用于覆盖默认符号 (需 \texttt{enumitem})。
    \item \code{\bullet}: 默认的圆点符号。
\end{itemize}
...
\begin{itemize}
    \item \code{\begin{enumerate}}: 开启有序列表环境。
    \item \code{label=\arabic*}: 设置标签样式，\code{\arabic*} 代表阿拉伯数字 (1, 2, 3)。
    \item \code{\alph*}: 小写字母 (a, b, c)。
    \item \code{\Roman*}: 大写罗马数字 (I, II, III)。
...
\begin{itemize}
    \item \code{\item[Label]}: 方括号内的内容会作为标签加粗显示。
\end{itemize}
...
\begin{itemize}
    ...
    \item \code{label=\textcolor{color}{symbol}}: 自定义标签颜色和符号。
\end{itemize}
...
\begin{itemize}
    \item \code{\square}: 空方框符号 (需 \texttt{amssymb})。
    \item \code{\boxtimes}: 带叉方框符号。
    \item \code{\checkmark}: 对勾符号。
\end{itemize}
