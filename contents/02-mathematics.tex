...
\begin{itemize}
    \item \code{\\lim}, \code{\\sum}, \code{\\int}: 分别表示极限、求和、积分。
    \item \code{\_\{...\}}: 下标 (Lower limit)。
    \item \code{\^\{...\}}: 上标 (Upper limit)。
    \item \code{\\to}: 趋近箭头 ($\to$)。
    \item \code{\\infty}: 无穷符号 ($\infty$)。
    \item \code{\\,}: 插入一个小间距 (thin space)，常用于微分算子 $dx$ 前。
\end{itemize}
...
\begin{itemize}
    \item 希腊字母: \code{\\alpha}, \code{\\beta}, \code{\\Delta} (大写), 等。
    \item 逻辑符号: \code{\\forall} ($\forall$), \code{\\exists} ($\exists$)。
    \item 集合符号: \code{\\in} ($\in$), \code{\\subseteq} ($\subseteq$), \code{\\cup} ($\cup$), \code{\\cap} ($\cap$)。
    \item \code{\\mathbb}: 黑板粗体，常用于表示数集 (需 \code{amssymb} 宏包)。
\end{itemize}
