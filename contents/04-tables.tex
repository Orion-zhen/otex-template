...
\begin{itemize}
    \item \textbf{列格式 (Column Specifiers)}: 定义在 \code{\textbackslash begin\{tabular\}} 后的花括号中。
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \code{\textbackslash \textbackslash}: 结束当前行 (Ends the current row).
    \end{itemize}
    \item \code{booktabs}: 提供 \code{\textbackslash toprule}, \code{\textbackslash midrule}, \code{\textbackslash bottomrule} 以绘制不同粗细的专业横线，替代标准的 \code{\textbackslash hline}。
\end{itemize}
...
\subsubsection{合并列 (\textbackslash multicolumn)}

合并列通过 \code{\textbackslash multicolumn} 命令实现。它不仅用于合并，也常用于改变单个单元格的对齐方式（例如表头居中，数据居左）。

Column merging is done via \code{\textbackslash multicolumn}. It is also used to change the alignment of a single cell.
...
\subsubsection{合并行 (\textbackslash multirow)}

合并行需要 \code{multirow} 宏包，命令为 \code{\textbackslash multirow}。
...
当使用了 \code{\textbackslash multirow\{2\}\{...\}} 时，当前行占据了位置。\textbf{在下一行对应的位置必须留空}（即直接写 \code{\&} 而不填内容），否则内容会重叠。

When using \code{\textbackslash multirow}, the corresponding cell in the \textbf{next rows} must be left empty.
...
注意使用 \code{\textbackslash cline\{2-2\}} 来只画第2列的横线，否则会穿过 "Common" 文本。

Notice using \code{\textbackslash cline\{2-2\}} to draw a horizontal line only across the 2nd column.
