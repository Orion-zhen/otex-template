...
\begin{itemize}
    \item \code{\\includegraphics[options]\{file\}}: 插入图片。
    \item \code{width=0.4\\textwidth}: 设置宽度为文本宽度的 40\%。
...
\begin{itemize}
    ...
    \item \code{\\hfill}: 在两个子图之间插入弹性空格以撑满行宽。
\end{itemize}
...
\begin{itemize}
    \item \code{\\begin\{wrapfigure\}\{align\}\{width\}}: 开启环绕环境。
