...
| `cm` | 厘米 (绝对单位) | `2.5cm` |
| `mm` | 毫米 (绝对单位) | `10mm` |
| `pt` | 点 (大约 1/72 英寸) | `12pt` |
| `em` | 相对于当前字体的字宽 (实用的相对单位) | `2em` (约为两个汉字的宽度) |
| `ex` | 相对于当前字体的小写字母 x 的高度 | `1ex` |

### 水平间距

- `\hspace{长度}`：插入指定长度的水平空白。
- `\hspace*{长度}`：即使在行首也会插入空白。
- `\hfill`：插入可伸缩的填充空白，常用于将内容推向行首或行尾。
- `\quad`：插入相当于当前字体大小的空白（通常是一个字宽）。
- `\qquad`：插入两个 `\quad` 的宽度。

```latex
左侧文字 \hspace{2cm} 右侧文字
左侧内容 \hfill 右侧内容
\textbf{姓名：}\quad \underline{\hspace{3cm}}
```

### 垂直间距

- `\vspace{长度}`：插入指定长度的垂直空白。
- `\vspace*{长度}`：即使在页首也会插入空白。
- `\vfill`：插入可伸缩的垂直填充空白，常用于封面垂直居中。
- `\\[长度]`：在换行时指定行间距偏移量。
...
- `\par`：显式地开始一个新段落（等同于在源代码中空一行）。
- `\indent`：强制产生首行缩进。
- `\noindent`：强制取消当前段落的首行缩进。
- `\setlength{\parindent}{长度}`：设置全局首行缩进宽度（如 `2em`）。
- `\setlength{\parskip}{长度}`：设置段落之间的间距。

```latex
\noindent 这段文字没有首行缩进。 \par
\indent 强制产生首行缩进。

\setlength{\parindent}{2em} % 常用于中文排版
\setlength{\parskip}{0.5ex plus 0.2ex minus 0.1ex} % 设置可变段间距
```

> **提示**：在使用 `\vspace` 或 `\hspace` 时，如果在页面/行开头位置被忽略，请尝试使用带星号的版本（如 `\vspace*`）。
